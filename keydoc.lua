...
local mycolor_key =   "#cb4b1c" -- beautiful.colors.red:sub(1,-3) --beautiful.bg_urgent:sub(1,-3)  
local mycolor_help =  "#fdf6e3"
local mycolor_group = "#dc322f"
    ...
    for _, key in ipairs(keys) do
        if doc[key] then
            ...
            result[group] = (result[group] or "") ..
               '<span font="DejaVu Sans Mono 10" color="' ..  mycolor_key.. '"> ' ..
               string.format("%" .. (longest - unilen(skey)) .. "s  ", "") .. skey ..
               '</span>  <span color="' .. mycolor_help .. '">' ..
               help .. '</span>\n'
        end
    end
    return result
...
function display()
    ...
    for group, res in pairs(strings) do
        ...
        result = result ..
            '<span weight="bold" color="' .. mycolor_group .. '">' ..
            group .. "</span>\n" .. res
        end
